...
// Чтобы чито-то на странице изменилось в React компонентах, то есть произошёл re-render (перерисовка)
// есть два основных способа: 1 - чтобы изменился props, 2 - чтобы изменилось внутринее состояние
...
function Counter() {
  ...
  // прии вызове хука useState мы должны передать начальное значение того, что мы
  // планируем хранить в стейте и затем изменять
  // хук возвращает массив из двух значений(всегда):
  // 1 - это и есть переменная, которая бувдет со временем изменяться
  // изменить state
